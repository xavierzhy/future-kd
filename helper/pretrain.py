...
def init(model_s, model_t, init_modules, criterion, train_loader, logger, opt):
    ...
    if opt.model_s in ['resnet8', 'resnet14', 'resnet20', 'resnet32', 'resnet44', 'resnet56', 'resnet110',
                       'resnet8x4', 'resnet32x4', 'wrn_16_1', 'wrn_16_2', 'wrn_28_4', 'wrn_40_1', 'wrn_40_2'] and \
            opt.distill == 'factor':
        lr = 0.01
    else:
    ...
    for epoch in range(1, opt.init_epochs + 1):
        ...
        for idx, data in enumerate(train_loader):
            ...
            # TODO: check the each
            is_preact = (opt.distill in ['abound', 'each-abound', 'overhaul', 'each-overhaul'])
            each = (opt.distill in ['each-abound', 'each-overhaul'])
            feat_s, _ = model_s(input, is_feat=True, is_preact=is_preact, each=each)
            with torch.no_grad():
                feat_t, _ = model_t(input, is_feat=True, is_preact=is_preact, each=each)
                feat_t = [f.detach() for f in feat_t]

            if opt.distill == 'abound' or opt.distill == 'each-abound':
                # init_modeules[0] == connector
                g_s = init_modules[0](feat_s[1:-1])
                g_t = feat_t[1:-1]
                loss_group = criterion(g_s, g_t)
                # hyper-parameter for each
                # loss = sum(loss_group)
                loss = sum(loss_group) / len(g_s) * 3
            elif opt.distill in ['overhaul', 'each-overhaul']:
                # overhaul do not need pretrain
                g_s = init_modules[0](feat_s[1:-1])
                g_t = feat_t[1:-1]
                BN = model_t.get_bn_before_relu()
                loss_group = criterion(g_s, g_t, BN)
                loss = sum(loss_group)
