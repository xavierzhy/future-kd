...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_last=False, each=False):
...
class ResNet(nn.Module):
    ...
    def get_bn_before_relu(self, each=False):
        if each:
    ...
    def forward(self, x, is_feat=False, is_preact=False, each=False):
        ...
        if each:
            ...
            features.append(x)
            x = self.fc(x)
            # assert len(features) == (self.depth - 2) // 2 + 2
            return features, x
        ...
        if is_feat:
            if is_preact:  # before the activate
