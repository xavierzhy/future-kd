...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, is_preact=False, each=False):
        x1, x2 = self.split(x)
        ...
        if (self.is_last and not each) or is_preact:
            return out, preact
        else:
            return out
...
class DownBlock(nn.Module):
    ...
    def forward(self, x, is_preact=False, each=False):
        ...
        preact1 = self.bn2(self.conv2(out1))
        out1 = F.relu(preact1)
        # right
        out2 = F.relu(self.bn3(self.conv3(x)))
        out2 = self.bn4(self.conv4(out2))
        preact2 = self.bn5(self.conv5(out2))
        out2 = F.relu(preact2)
        # concat
        preact = torch.cat([preact1, preact2], 1)
...
class ShuffleNetV2(nn.Module):
    ...
    def forward(self, x, is_feat=False, is_preact=False, each=False):
        ...
        if each:
            features = [f0]
            for layers in [self.layer1, self.layer2, self.layer3]:
                for layer in layers:
                    if is_preact:
                        out, preact = layer(out, is_preact=is_preact, each=each)
                        features.append(preact)
                    else:
                        out = layer(out, is_preact=is_preact, each=each)
                        features.append(out)
